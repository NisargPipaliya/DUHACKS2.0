    ...
    else
    {
        if($password == $cpassword)
        {
            // $sql = "INSERT INTO `garden_owners` (`GID`, `NAME`, `onumber`, `ODURATION`, `AREA`, `CHARGEPERMONTH`, `pwd`) VALUES (NULL, '$name', '$num','$dur','$area','$cpm','$pwd');";
                ...
                $sql1="INSERT INTO `address`(`AID`, `BLOCK`, `STREET`, `LOCALITY`, `CITY`, `PINCODE`, `STATE`, `COUNTRY`) VALUES (NULL,'$block','$street','$loc','$city','$pin','$state','$country')";
                $dem="SELECT 'AID' FROM `address` where `BLOCK`='$block',`STREET`='$street',`LOCALITY`='$loc',`CITY`='$city',`PINCODE`='$pin',`STATE`='$state',`COUNTRY`='$country'";
                $aid=mysqli_query($conn,$dem);
                $sql="INSERT INTO `customer`(`CID`, `NAME`, `AID`, `pwd`) VALUES (NULL,'$name',1,'$pwd')";
...
<style>
    ...
    .form_wrap input[type="text"] , [type="password"]{
        width: 100%;
            ...
            <div class="registration_form">
                ...
                <form action="signupcustomer.php" method="post">
                    <div class="form_wrap">
                        ...
                        <div class="input_wrap">
                            <button type="submit" name="submit" class="submit_btn">Register Now</button>
                        </div>
                    </div>
                </form>
            </div>
